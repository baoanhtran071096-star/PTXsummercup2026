...
export default function PublicBetaDemoPage() {
  ...
  const [events, setEvents] = useState([
    { minute: 14, type: 'GOAL_SCORED', player: 'Kylian mBAppé (#9.5)', team: 'FC QUẢN LÝ', details: 'Sút xa đẹp mắt 25m' },
    { minute: 32, type: 'YELLOW_CARD', player: 'VERL (#11)', team: 'FC QUẢN LÝ', details: 'Phạm lỗi nguy hiểm' },
    { minute: 68, type: 'GOAL_SCORED', player: 'Erling HaaTháiland (#9)', team: 'FC QUẢN LÝ', details: 'Đánh đầu cận thành' }
  ]);
    ...
    return (
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, left: 0, background: '#0b0f19', zIndex: 99999, display: 'grid', placeItems: 'center', textAlign: 'center' }}>
        {splashStage === 'INTRO' ? (
          ...
          <div>
            <h2 style={{ fontSize: '1.5rem', color: 'var(--accent-cyan)', fontWeight: '700', marginBottom: '20px' }}>Loading Legacy Tournament Data...</h2>
            <div style={{ display: 'flex', gap: '20px', fontSize: '1.1rem', fontFamily: 'var(--font-mono)' }}>
              <span style={{ color: initItems.teams ? 'var(--accent-green)' : 'var(--text-muted)' }}>{initItems.teams ? '✓' : '○'} 3 Real Teams (Sân 5)</span>
  ...
  return (
    <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '16px' }}>
      {/* 🚧 Transparent Public Beta Banner - Legacy Data Policy Compliant */}
      <div style={{ background: 'linear-gradient(90deg, rgba(255,183,3,0.2) 0%, rgba(255,77,109,0.15) 100%)', border: '1px solid rgba(255,183,3,0.4)', borderRadius: '12px', padding: '12px 20px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <strong style={{ color: 'var(--accent-gold)' }}>PTX Platform Public Beta — Legacy Data Policy Compliant</strong>
            <span style={{ color: 'var(--text-muted)', marginLeft: '10px', fontSize: '0.9rem' }}>Đã chuẩn hóa 100% dữ liệu gốc: 3 Đội bóng thật, Thể thức Sân 5 & Roster 26 Cầu thủ thật!</span>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '16px 24px', marginBottom: '16px', background: 'rgba(0, 242, 254, 0.15)', border: '1px solid var(--accent-cyan)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <span style={{ fontWeight: '600' }}>
              {storyTimelineStep === 1 && '⏰ 08:00 — Ban Tổ Chức mở PTX Platform kiểm tra 3 Đội thật (Thể thức Sân 5), Lịch đấu & Sân thi đấu'}
              {storyTimelineStep === 2 && '⏰ 08:30 — Quản lý kiểm tra Roster 26 cầu thủ & Size áo đấu (Kylian mBAppé #9.5 Size 2XL)'}
              {storyTimelineStep === 3 && '⏰ 09:00 — Trọng tài bắt đầu trận đấu, ghi nhận Goal từ Kylian mBAppé & Live Score nhảy 3-1'}
              {storyTimelineStep === 4 && '⏰ 09:45 — Khán giả truy cập xem tỉ số trực tiếp & Bảng xếp hạng thay đổi'}
              {storyTimelineStep === 5 && '⏰ 10:00 — Ban tổ chức upload ảnh trận đấu lên DAM Gallery & Vinh danh Title Sponsor (Vinamilk, Viettel)'}
              {storyTimelineStep === 6 && '⏰ 17:00 — Bế mạc giải đấu, Hall of Fame vinh danh FC QUẢN LÝ & Vua Dội Bom Golden Boot Kylian mBAppé!'}
            </span>
          </div>
      ...
      <header className="glass-panel" style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Enterprise Tournament Engine v1.0.1 • Legacy Data Compliant</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <section className="glass-panel" style={{ padding: '36px', marginBottom: '24px', position: 'relative', overflow: 'hidden', borderRadius: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', alignItems: 'center', zIndex: 2, position: 'relative' }}>
          <div>
            <span style={{ color: 'var(--accent-cyan)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '2px', fontSize: '0.85rem' }}>GIẢI BÓNG ĐÁ SÂN 5 • DATA MIGRATION VERIFIED</span>
            ...
            <p style={{ color: 'var(--text-main)', fontSize: '1.05rem', marginBottom: '20px', lineHeight: 1.6 }}>
              Hệ thống điều hành bóng đá tự động hóa 100%: <strong>Chuẩn 3 Đội Bóng Thật (FC QUẢN LÝ, FC VỀ NHÌ, FC TIÊN PHONG)</strong>, <strong>Thể thức Thi đấu Sân 5</strong>, <strong>Live Match Console realtime</strong> và <strong>Jersey Management Module</strong>.
            </p>

            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <button className="btn-primary" onClick={() => setActiveTab('JERSEY_MGMT')}>👕 Roster & Áo Đấu (26 Cầu Thủ Thật)</button>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border-glass)' }}>
                ...
                <h4 style={{ fontWeight: '700', color: 'var(--accent-cyan)' }}>Legacy Data Policy First</h4>
                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>Bám sát 100% dữ liệu gốc: 3 Đội thật, Thể thức Sân 5 & 26 Cầu thủ thật.</p>
              </div>
              <div style={{ background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border-glass)' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div style={{ background: 'rgba(0,242,254,0.1)', border: '1px solid rgba(0,242,254,0.3)', padding: '20px', borderRadius: '12px', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>FC QUẢN LÝ — 5 Assists</span>
            </div>
          </div>
        </div>
      )}
